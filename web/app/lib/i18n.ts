export const messages = {
  en: {
    ...
    certificates: {
      ...
      rotateTitle: (label: string, domain: string) => `Rotate ${label} for ${domain}?`,
      rotateDescription: (label: string) =>
        `The existing ${label} will stop working immediately.`,
      confirmRotate: "Rotate URL",
  ...
  "zh-CN": {
    ...
    certificates: {
      ...
      confirmDelete: "确认删除",
      rotate: "轮换",
      rotateLabel: (label: string) => `轮换${label}`,
      rotateTitle: (label: string, domain: string) => `轮换 ${domain} 的${label}？`,
      rotateDescription: (label: string) => `现有${label}将立即失效。`,
      confirmRotate: "确认轮换",
      brandLine: "证书控制",
      apiOnline: "API 在线",
      logout: "退出登录",
